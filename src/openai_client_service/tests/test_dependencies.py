...
def test_get_subject_success() -> None:
    ...
    assert asyncio.run(dependencies.get_subject("alice")) == "alice"


def test_get_subject_missing_header() -> None:
    """Missing subject headers should raise HTTP 401."""
    with pytest.raises(dependencies.HTTPException) as exc:
        asyncio.run(dependencies.get_subject(None))
...
def test_session_lifecycle() -> None:
    ...
    assert asyncio.run(dependencies.get_authenticated_subject(session_id=SESSION_ID)) == "bob"
    dependencies.destroy_session_for_testing(SESSION_ID)
    with pytest.raises(dependencies.HTTPException):
        asyncio.run(dependencies.get_authenticated_subject(session_id=SESSION_ID))


def test_get_ai_client_uses_subject(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    subject = asyncio.run(dependencies.get_authenticated_subject(SESSION_ID))
